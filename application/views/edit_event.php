<?php 
	...
	list($hr,$mn, $sc) = explode(':', $event['time']);
	
	for($i = 0; $i< 60; $i++){
		if($i < 24){
			$h .= '<option '.(($i == $hr)? "selected" : "").' value="'.(($i > 9)? $i : "0$i").'">'.(($i > 9)? $i : "0$i").'</option>';
		}
		$m .= '<option '.(($i == $mn)? "selected" : "").' value="'.(($i > 9)? $i : "0$i").'">'.(($i > 9)? $i : "0$i").'</option>';
	}
	$h .= '</select>';
	$m .= '</select>';
?>
<div style="width:500px; height:135px; overflow:auto; color:#000000; margin-bottom:20px;" align="center">
	<h4>Edit event for <?php echo "$day $month $year"?></h4>
	...
	<table>
		...
		<tr><td>Event <span class="require">*</span></td><td>:</td><td><input type="text" name="event" id="event" value="<?php echo $event['event']?>" maxlength="50" size="50" /></td></tr>
	...
	$('.save').click(function(){
		if($('#event').val().length > 0){
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: "<?php echo site_url("evencal/do_edit");?>",
				data:{<?php echo "id:{$event['id']},year:$year,mon:$mon,day:$day";?>, hour:$('#hour').val(), minute: $('#minute').val(), event:$('#event').val()},
